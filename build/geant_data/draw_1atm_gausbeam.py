...
fnames = ["mu-_mom%d_bite5_Ne1atm_gausbeam"%(int(mom)) for mom in Mom]
...
for m,i,st in zip(Mom,i_150kw,stop_rate):
    print "%d %.1f %.4f %.2f"%(m,i,st,st*i)
...
plt.ylabel("# of stopped $\mu^-$ on Ne 1atm target [/sec]")
plt.legend()
plt.tight_layout()
plt.savefig("Ne1atm_stopmu_per_sec.pdf")
...
plt.legend()
plt.tight_layout()
plt.savefig("Activity_Ne1atm_stopmu_Bq.pdf")
